...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
      { id: 'flash-sale-list', path: '/flash-sale', label: 'Flash Sale', icon: <FlameIcon /> },
      { id: 'flash-sale-schedule', path: '/flash-sale/schedule', label: 'Lịch hẹn giờ', icon: <ClockIcon /> },
    ],
  },
  {
    id: 'ads',
    ...
    children: [
      { id: 'ads-campaigns', path: '/ads', label: 'Quảng cáo', icon: <AdsIcon /> },
      { id: 'ads-keywords', path: '/ads/keywords', label: 'Từ khóa', icon: <KeywordIcon /> },
      { id: 'ads-tracking', path: '/ads/tracking', label: 'Theo dõi', icon: <TrackingIcon /> },
    ],
  },
  {
    id: 'account-health',
  ...
  },
  {
    ...
    children: [
      { id: 'profile-info', path: '/profile', label: 'Thông tin cá nhân', icon: <UserIcon /> },
      { id: 'profile-shops', path: '/profile/shops', label: 'Quản lý Shop', icon: <ShopIcon /> },
    ],
  },
];
...
function ShopSelector() {
  ...
  const currentShop = shops.find((s) => s.shop_id === selectedShopId) ||
    (token?.shop_id ? { shop_id: token.shop_id, shop_name: `Shop ${token.shop_id}`, region: 'VN', shop_logo: null } : null);
    ...
    <div className="relative z-50">
      <button
        ...
        className={cn(
          ...
          shops.length > 1 ? 'bg-orange-50 border-orange-200 hover:bg-orange-100 cursor-pointer' : 'bg-slate-50 border-slate-200 cursor-default'
        )}
      >
        <div className="w-6 h-6 bg-orange-100 rounded flex items-center justify-center overflow-hidden">
          {currentShop.shop_logo ? (
            <img src={currentShop.shop_logo} alt={currentShop.shop_name || ''} className="w-full h-full object-cover" />
          ) : (
            <ShopIcon />
          )}
        </div>
          ...
          <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-lg border border-slate-200 py-2 z-50 max-h-80 overflow-auto">
            ...
            {shops.map((shop) => (
              <button
                ...
                className={cn('w-full px-3 py-2 text-left flex items-center gap-3', shop.shop_id === selectedShopId ? 'bg-orange-50' : 'hover:bg-slate-50')}
              >
                <div className="w-8 h-8 bg-orange-100 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden">
                  {shop.shop_logo ? <img src={shop.shop_logo} alt={shop.shop_name || ''} className="w-full h-full object-cover" /> : <ShopIcon />}
                </div>
...
export default function DashboardLayout() {
  ...
  const { token, isLoading: isShopeeLoading } = useShopeeAuth();
  ...
  const getActiveMenu = () => {
    const directMatch = menuItems.find((m) => m.path === pathname);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          <img src="/logo_betacom.png" alt="BETACOM" className="w-24 h-24 mx-auto mb-6 animate-pulse" />
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col overflow-hidden">
      <div className="flex-1 flex overflow-hidden">
        {/* Sidebar */}
        <aside className={cn('bg-white border-r border-slate-200 flex flex-col transition-all duration-300 shadow-sm h-full', sidebarCollapsed ? 'w-16' : 'w-64')}>
          {/* Logo */}
                ...
                <div key={item.id}>
                  <button
                    ...
                    className={cn(
                      ...
                      isActive ? 'bg-gradient-to-r from-orange-500 to-red-500 text-white shadow-lg shadow-orange-500/25' : 'text-slate-600 hover:bg-slate-50',
                      sidebarCollapsed && 'justify-center px-2'
                    )}
                    title={sidebarCollapsed ? item.label : undefined}
                  >
                    <span className={cn('transition-colors', isActive ? 'text-white' : 'text-slate-400 group-hover:text-orange-500')}>{item.icon}</span>
                    {!sidebarCollapsed && (
                      <>
                      ...
                      {item.children!.map((child) => (
                        <button
                          ...
                          className={cn(
                            ...
                            activeMenu === child.id ? 'bg-orange-50 text-orange-600 font-medium' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'
                          )}
                        >
                          <span className={cn('w-4 h-4', activeMenu === child.id ? 'text-orange-500' : 'text-slate-400')}>{child.icon}</span>
          ...
          <div className="p-2 border-t border-slate-100">
            <div className="relative">
              <button
                ...
                className={cn('w-full flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-50 transition-colors', sidebarCollapsed && 'justify-center')}
              >
                ...
                {!sidebarCollapsed && (
                  <div className="flex-1 text-left">
                    <p className="text-sm font-medium text-slate-700 truncate">{profile?.full_name || user?.email?.split('@')[0]}</p>
                    <p className="text-xs text-slate-400 capitalize">{profile?.work_type === 'fulltime' ? 'Full-time' : 'Part-time'}</p>
                  </div>
                )}
              </button>
        ...
        <main className="flex-1 overflow-auto">
          ...
          <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-30">
            ...
            <div className="flex items-center gap-3">{isShopConnected && <ShopSelector />}</div>
          </header>
